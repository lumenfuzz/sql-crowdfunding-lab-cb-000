...
"SELECT Projects.title, SUM(Pledges.amount) FROM Pledges INNER JOIN Projects ON Pledges.project_id = Projects.id GROUP BY Pledges.project_id HAVING SUM(Pledges.amount) > Projects.funding_goal ORDER BY Projects.title ASC;"
end
